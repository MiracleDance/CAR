...
class ConditionDatasetFolder(VisionDataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        parts = path.split('/')
        condition_path = os.path.join(self.condition_path, parts[-3], parts[-2], parts[-1])  # condition's path should align with the image's path
        ...
        if self.transform is not None and not self.train:
            ...
            condition_sample = self.transform(condition_sample)
            sample = self.transform(sample)
            i, j, h, w = transforms.RandomCrop.get_params(sample, output_size=(256, 256))
